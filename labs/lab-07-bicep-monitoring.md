...
**The first gap is how your infrastructure got there.** Every Azure resource you have used so far was created by hand in the portal. That is *click-ops* — the same "works on my machine" problem, one layer down. Nobody can review a click. Nobody can repeat it reliably. And when someone quietly changes a setting six weeks from now, nothing records it — that silent divergence is *configuration drift*. Infrastructure as Code fixes this by treating cloud resources like software: repeatable, reviewable, and version-controlled.
